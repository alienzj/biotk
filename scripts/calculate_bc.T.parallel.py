...
def calculate_bray_curtis_dist(input_file, output_file, threads):
    ...
    rownames = list(PROFILE_DF.index.unique)
    for i in range(0, len(rownames)):
        for j in range(i, len(rownames)):
            subject_i = rownames[i]
            subject_j = rownames[j]
...
def main():
    ...
    parser.add_argument(
        "--profile",
        type=str,
        help="input profile, rowname is subject, colname is feature, format: csv"
    )
    parser.add_argument(
        "--output",
        type=str,
